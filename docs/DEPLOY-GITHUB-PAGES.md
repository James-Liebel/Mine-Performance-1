...
- **Build**: The workflow sets `GITHUB_PAGES_DEPLOY=1` and `BASE_PATH=/<repo-name>` (e.g. `/Mine-Performance-1`), then runs a static export. API routes and dynamic admin routes are not supported; the workflow temporarily moves `app/api` and `app/admin` aside so the build succeeds. The exported site is in `out/`.
...
```bash
# Unix/macOS
GITHUB_PAGES_DEPLOY=1 BASE_PATH=/Mine-Performance-1 npm run build

# Windows (PowerShell)
$env:GITHUB_PAGES_DEPLOY="1"; $env:BASE_PATH="/Mine-Performance-1"; npm run build
```
...
Then serve the `out/` folder (e.g. `npx serve out` or any static server). Replace `Mine-Performance-1` with your repo name for MP2/MP3.
