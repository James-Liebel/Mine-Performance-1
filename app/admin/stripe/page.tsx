      ...
      <section className="card card-elevated admin-analytics-section admin-analytics-section--wide" style={{ marginBottom: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap', marginBottom: '1rem', fontSize: '0.9rem' }}>
          ...
          <div>
            ...
            <div className="admin-metric-value">${Math.round(avg).toLocaleString()}</div>
          </div>
          <div>
            <div className="admin-metric-label">Best month:</div>
            <div className="admin-metric-value">{best.month} &middot; {formatMoneyShort(best.revenue)}</div>
          </div>
        </div>

        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'flex-end', height: 140, paddingBottom: '0.5rem', borderBottom: '1px solid var(--border)', marginBottom: '0.5rem' }}>
          {DEMO_STRIPE_MONTHS.map((m) => {
            const ratio = m.revenue / tickMax;
            const h = 20 + ratio * 100;
            const isBest = m.month === best.month;
            return (
              <div key={m.month} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', gap: '0.25rem', fontSize: '0.75rem' }}>
                <div
                  style={{ width: '100%', maxWidth: 22, borderRadius: '6px 6px 0 0', background: isBest ? 'var(--accent)' : 'var(--accent-soft, #93c5fd)', height: `${h}px` }}
                  title={`${m.month}: ${formatMoneyShort(m.revenue)} · ${m.payments} payments`}
                />
                <span style={{ whiteSpace: 'nowrap' }}>
                  {m.month}
                  <span style={{ display: 'block', fontSize: '0.7rem', color: 'var(--text-muted)' }}>{formatMoneyShort(m.revenue)}</span>
                </span>
              </div>
            );
          })}
        </div>
        <p className="text-muted" style={{ margin: 0, fontSize: '0.85rem' }}>Demo data only. Actual payments and payouts appear in your Stripe Dashboard.</p>
      </section>
      ...
      <section className="card card-elevated admin-analytics-section">
        ...
        <ul className="admin-metrics-list">
          <li>
            ...
            <span className="admin-metric-value">demo revenue by month and an example payments summary.</span>
          </li>
          <li>
            <span className="admin-metric-label">For live data:</span>{' '}
            <span className="admin-metric-value">go to the Stripe Dashboard for real payments, refunds, disputes, and exports.</span>
          </li>
        </ul>
        <div style={{ marginTop: '1rem' }}>
          <Link href="https://dashboard.stripe.com/" target="_blank" rel="noreferrer" className="btn btn-primary">Open Stripe Dashboard</Link>
        </div>
      </section>
    </div>
  );
